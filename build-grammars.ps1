...
param(
    ...
    [ValidateSet("15.0","16.0","17.0","18.0")]
...
function Find-VcVarsAll {
    ...
    $vsVersionRange = Get-VsVersionRange
    if ($vsVersionRange) {
        $ip = & $vswhere -version $vsVersionRange -products * -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath 2>$null
    } else {
...
function Import-VcEnvironment {
    ...
    if ($WindowsSdkVersion) {
        $vcvarsArgs = "$Arch $WindowsSdkVersion"
    } else {
        $vcvarsArgs = $Arch
    }
    Write-Host "Importing MSVC environment ($vcvarsArgs) ..."
    $batContent = "@call `"$vcvars`" $vcvarsArgs >nul 2>&1`r`n@set > `"$env:TEMP\vcvars_env.txt`"`r`n"
...
function Get-VcArch {
    switch ($Platform) {
        "x64"    { return "amd64" }
        "x86"    { return "x86" }
        "ARM"    { return "amd64_arm" }
        "ARM64"  { return "amd64_arm64" }
        default  { return "amd64" }
    }
}

function Get-VsVersionRange {
    switch ($VisualStudioVersion) {
        "15.0" { return "[15.0,16.0)" }
        "16.0" { return "[16.0,17.0)" }
        "17.0" { return "[17.0,18.0)" }
        "18.0" { return "[18.0,19.0)" }
    }
}
...
function Build-GrammarDll {
    ...
    if ($Configuration -eq "Release") {
        $cflags += @("/O2","/MT","/DNDEBUG","/GL")
    } else {
        $cflags += @("/Od","/MTd","/D_DEBUG","/Zi")
    }
    ...
    foreach ($src in $sources) {
        ...
        $sw.Stop()
        Write-Host "  Compile Time = $($sw.Elapsed)"
    }
    ...
    if ($XP) {
        $linkArgs += " /SUBSYSTEM:WINDOWS,5.01"
    }
    $linkArgs += $objFiles
